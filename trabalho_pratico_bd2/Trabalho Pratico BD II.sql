...
-- 1: --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
delimiter $
create procedure proc_qteFuncionarios(tipo integer) 
begin 
if tipo = 0 then select count(*) as vQte from funcionario;
elseif tipo = 1 then select count(*) as vQte from funcionario where funcionario.situacao = 0;
end if;
end$
delimiter ;
...
DECLARE n1 INT DEFAULT 0; DECLARE n2 INT DEFAULT 0; DECLARE passo int; WHILE verifica = 2 DO SET contador = contador + 1;
...
create procedure proc_qteFuncionariosCPFinv() select count(cpf) as vQte from funcionario join pessoa
where funcionario.idpessoa = pessoa.id and (func_validaCPF(pessoa.cpf) = 0);
...
where pessoa.sexo = sexo and pessoa.id = funcionario.idpessoa and funcionario.situacao = 0);
...
return (select sum(salario) from funcionario join cargo join departamento
where funcionario.idcargo = cargo.id and cargo.iddepartamento = departamento.id and funcionario.situacao = 0 and iddepartamento = departamento.id);
...
return (select avg(salario) from funcionario join cargo join departamento
where funcionario.idcargo = cargo.id and cargo.iddepartamento = departamento.id and funcionario.situacao = 0 and iddepartamento = departamento.id);
...
return (select avg(idade) from (select func_idade(pessoa.dtnascimento) as idade from pessoa join funcionario join cargo join departamento
where iddepartamento = departamento.id and funcionario.idpessoa = pessoa.id and funcionario.idcargo = cargo.id 
and cargo.iddepartamento = departamento.id and funcionario.situacao = 0) as idade);
